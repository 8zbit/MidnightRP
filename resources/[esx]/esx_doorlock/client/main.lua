...
AddEventHandler('esx_doorlock:test', function(keycards)
	...
	if keycards > 0 or ESX.PlayerData.job.name == "police" then
		...
		exports['progressBars']:startUI(10000, "Jamming Circuits")
...
Citizen.CreateThread(function()
	while true do
		...
		local playerCoords = GetEntityCoords(PlayerPedId())
		local ped = PlayerPedId()
		local distance = GetDistanceBetweenCoords(playerCoords, Config.textCoords.x,Config.textCoords.y,Config.textCoords.z, true)
		
		if distance <= 2.0 then
			
			DrawText3D(Config.textCoords, "Press [~r~E~w~] to power down the grid", 2.0)
