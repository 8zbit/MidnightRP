...
ESX.RegisterServerCallback("dubs:retrieveVehicles", function(source, cb)
	local src = source
	local identifier = ESX.GetPlayerFromId(src)["identifier"]

    MySQL.Async.fetchAll("SELECT seller, vehicleProps, price FROM vehicles_for_sale2", {}, function(result)
        local vehicleTable = {}
...
ESX.RegisterServerCallback("dubs:isVehicleValid", function(source, cb, vehicleProps, price)
	local src = source
	...
	RetrievePlayerVehicles(xPlayer.identifier, function(ownedVehicles)
		...
		for id, v in pairs(ownedVehicles) do
			...
			if Trim(plate) == Trim(v.plate) and #Config.VehiclePositions ~= VehiclesForSale then
                ...
                MySQL.Async.execute("INSERT INTO vehicles_for_sale2 (seller, vehicleProps, price) VALUES (@sellerIdentifier, @vehProps, @vehPrice)",
                    {
                ...
                TriggerClientEvent("dubs:refreshVehicles", -1)
...
ESX.RegisterServerCallback("dubs:buyVehicle", function(source, cb, vehProps, price)
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
	...
	if xPlayer.getAccount("bank")["money"] >= price or price == 0 then
		...
		MySQL.Async.fetchAll('SELECT seller FROM vehicles_for_sale2 WHERE vehicleProps LIKE "%' .. plate .. '%"', {}, function(result)
		...
		MySQL.Async.execute('DELETE FROM vehicles_for_sale2 WHERE vehicleProps LIKE "%' .. plate .. '%"', {})
