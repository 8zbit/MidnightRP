...
AddEventHandler("t1ger_drugs:GetSelectedJob", function(drugType,BuyPrice,minReward,maxReward)
	...
	if xPlayer.getMoney() >= BuyPrice then
		...
		if drugType == "opium" then
			label = "opium"
		elseif drugType == "mdma" then
			label = "mdma"
		elseif drugType == "hashish" then
			label = "hashish"
		end	
...
AddEventHandler("t1ger_drugs:sellDrugs", function()
	local xPlayer = ESX.GetPlayerFromId(source)
	local hashish = xPlayer.getInventoryItem(Config.hashishDrug).count
	local mdma = xPlayer.getInventoryItem(Config.mdmaDrug).count
	local opium = xPlayer.getInventoryItem(Config.opiumDrug).count
	local drugamount = 0
	local price = 0
	local drugType = nil
	
	if hashish > 0 then
		drugType = Config.hashishDrug
		if hashish == 1 then
			drugamount = 1
		elseif hashish == 2 then
			drugamount = math.random(1,2)
		elseif hashish == 3 then	
			drugamount = math.random(1,3)
		elseif hashish >= 4 then	
			drugamount = math.random(1,4)
		end
		
	elseif mdma > 0 then
		drugType = Config.mdmaDrug
		if mdma == 1 then
			drugamount = 1
		elseif mdma == 2 then
			drugamount = math.random(1,2)
	...
	if drugType==Config.hashishDrug then
		price = math.random(Config.hashishSale.min,Config.hashishSale.max) * 10 * drugamount
	elseif drugType==Config.mdmaDrug then
		price = math.random(Config.mdmaSale.min,Config.mdmaSale.max) * 10 * drugamount
...
AddEventHandler("t1ger_drugs:canSellDrugs", function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer ~= nil then
		local soldAmount = (xPlayer.getInventoryItem("opium1g").count > 0 or xPlayer.getInventoryItem("mdma1g").count > 0 or xPlayer.getInventoryItem("hashish4g").count > 0) and CheckSoldAmount(xPlayer.getIdentifier()) < Config.maxCap
