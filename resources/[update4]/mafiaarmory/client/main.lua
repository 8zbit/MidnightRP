...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not Config.Onlymafiamen or (Config.Onlymafiamen and ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "mafia") then
...
OpenmafiaArmory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "mafia_armory_menu",
		{
			title    = "mafia Armory",
			align    = "center",
			elements = elements
...
OpenWeaponStorage = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "mafia_armory_weapon_menu",
		{
			title    = "mafia Weapon Armory",
			align    = "center",
			elements = elements
		},
	function(data, menu)
		...
		if DoesEntityExist(closestPed) and closestPedDst <= 5.0 then
			...
			TriggerServerEvent("mafiaarmory:giveWeapon", weaponHash)
		end
...
RefreshPed = function(spawn)
	...
	ESX.TriggerServerCallback("mafiaarmory:pedExists", function(Exists)
