...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'medikit' and itemName ~= 'bandage' and itemName ~= 'bodybandage' and itemName ~= 'legbrace' and itemName ~= 'armbrace' and itemName ~= 'neckbrace' and itemName ~= 'jello') then
		print(('esx_ambulancejob: %s attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
