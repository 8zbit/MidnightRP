...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('benny:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('benny:installMod')
AddEventHandler('benny:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('benny:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('benny:cancelInstallMod')
AddEventHandler('benny:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					if isRimMod then
						...
						TriggerServerEvent('benny:buyMod', price)
		...
		menu.close()
		TriggerEvent('benny:cancelInstallMod')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'mecano') or  not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
