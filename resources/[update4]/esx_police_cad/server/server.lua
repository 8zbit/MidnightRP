local dbReady = false
local crimal_record_query = "CREATE TABLE IF NOT EXISTS `criminal_records` ( `id` int(11) NOT NULL UNIQUE auto_increment, `offence` varchar(250) NOT NULL ,`user_id` varchar(250) NOT NULL, `officer_id` varchar(250) NOT NULL, `jail` int(11), `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(`id`))"
...
AddEventHandler('onMySQLReady', function ()
	MySQL.Async.execute(crimal_record_query, {}, function() end)
    MySQL.Async.execute(note_query, {}, function() end)
    MySQL.Async.execute(bolos_query, {}, function() end)
...
AddEventHandler('esx_police_cad:add-bolo', function(data)
    MySQL.Async.execute("INSERT into epc_bolos SET age = @age, height = @height, gender = @gender, type_of_crime = @type_of_crime, note = @note ", {
        ['@age'] = data.age,
        ['@height'] = data.height,
        ['@gender'] = data.gender,
