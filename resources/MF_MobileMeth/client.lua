            ...
            if not self.MethCook then
              ...
              if foundCook and foundCook ~= GetPlayerPed(-1) then
                if GetEntitySpeed(self.TruckSpawned) * 3.6 > self.MinSpeedForCook then 
                  ...
                  if not self.DidNotify then ESX.ShowNotification("Drive above 15KPH to begin the cook."); self.DidNotify = true; end
                end
              end
            else
              ...
              if self.CookFinished then
                ...
                if dist < self.DrawTextDist*2 then
                  ...
                  Utils.DrawText3D(pos.x,pos.y,pos.z, "Pres [~r~E~s~] deliver the meth.", 200)
...
function MFM:BeginCooking(driver)
  ...
  exports['progressBars']:startUI(math.floor(self.CookTimerA * 60 * 1000), "Preparing ingredients")
  Citizen.Wait(math.floor(self.CookTimerA * 60 * 1000) + 5000)

  if doCont then
    exports['progressBars']:startUI(math.floor(self.CookTimerB * 60 * 1000), "Cooking meth")
    Citizen.Wait(math.floor(self.CookTimerB * 60 * 1000) + 5000)
  else return; end

  if doCont then
    exports['progressBars']:startUI(math.floor(self.CookTimerC * 60 * 1000), "Allowing meth to set")
    Citizen.Wait(math.floor(self.CookTimerC * 60 * 1000) + 5000)
  else return; end

  if doCont then
    exports['progressBars']:startUI(math.floor(self.CookTimerD * 60 * 1000), "Packaging meth")
    Citizen.Wait(math.floor(self.CookTimerD * 60 * 1000) + 5000)
  else return; end

  if doCont then
    ESX.ShowNotification("You finished the cook.")
    TriggerServerEvent('MF_MobileMeth:FinishCook', self.Driver, true, "The cook has finished.")
    self.Driver = false
  end
end
