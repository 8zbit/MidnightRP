...
Citizen.CreateThread(function()
    ...
    if GUI.PhoneIsShowed then -- codes here: https://pastebin.com/guYd0ht4
      ...
      if IsDisabledControlJustReleased(0, 142) then -- MeleeAttackAlternate
        ...
        if IsControlPressed(0, 82) and (GetGameTimer() - GUI.Time) > 150 and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
        ...
        if IsControlPressed(0, 82) and (GetGameTimer() - GUI.Time) > 150 and ESX.PlayerData.job and ESX.PlayerData.job.name == 'offpolice' then
            if not ESX.UI.Menu.IsOpen('cadsystem', GetCurrentResourceName(), 'main') then
              ESX.UI.Menu.CloseAll()
              ESX.UI.Menu.Open('cadsystem', GetCurrentResourceName(), 'main')
            end
...
RegisterCommand("opencad", function(source, args, raw)
    if (GetGameTimer() - GUI.Time) > 150 and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' or (GetGameTimer() - GUI.Time) > 150 and ESX.PlayerData.job and ESX.PlayerData.job.name == 'offpolice' then
            if not ESX.UI.Menu.IsOpen('cadsystem', GetCurrentResourceName(), 'main') then
              ESX.UI.Menu.CloseAll()
              ESX.UI.Menu.Open('cadsystem', GetCurrentResourceName(), 'main')
            end
    ...
    exports['mythic_notify']:SendAlert('inform', 'Sa ei ole politsei')
  end
end)
