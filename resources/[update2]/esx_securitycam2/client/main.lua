...
AddEventHandler('esx_securitycam2:hasEnteredMarker', function (zone)
...
AddEventHandler('esx_securitycam2:hasExitedMarker', function(zone)
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_securitycam2:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_securitycam2:hasExitedMarker', LastZone)
			CurrentAction = nil
	...
	while true do
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
				{
					...
					if data.current.value == 'bankmenu' then
						...
						if bankHacked then
							...
							menuopen = false
							TriggerEvent('esx_securitycam2:freeze', true)
						end
		...
		if createdCamera ~= 0 then
			...
			if IsControlJustPressed(0, Keys["BACKSPACE"]) then
				...
				TriggerEvent('esx_securitycam2:freeze', false)
			end
...
function mycbpolice(success, timeRemaining)
	if success then
		...
		TriggerServerEvent('esx_securitycam2:setPoliceHackedState', true)
...
function mycbbank(success, timeRemaining)
	if success then
		TriggerEvent('mhacking:hide')
		TriggerServerEvent('esx_securitycam2:setBankHackedState', true)
		if Config.pNotify then
			TriggerEvent("pNotify:SendNotification", {
				text = _U('hacking_succeed'),
				type = "success",
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'UnHackBank' then
			...
			if bankHacked then
				TriggerServerEvent('esx_securitycam2:unhackanimserver')
				TriggerServerEvent('esx_securitycam2:setBankHackedState', false)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'UnHackPolice' then
			if policeHacked then
				TriggerServerEvent('esx_securitycam2:unhackanimserver')
				TriggerServerEvent('esx_securitycam2:setPoliceHackedState', false)
...
RegisterNetEvent('esx_securitycam2:freeze')
AddEventHandler('esx_securitycam2:freeze', function(freeze)
...
RegisterNetEvent('esx_securitycam2:unhackanim')
AddEventHandler('esx_securitycam2:unhackanim', function()
...
RegisterNetEvent('esx_securitycam2:setBankHackedState')
AddEventHandler('esx_securitycam2:setBankHackedState', function(state)
	bankHacked = state
end)

RegisterNetEvent('esx_securitycam2:setPoliceHackedState')
