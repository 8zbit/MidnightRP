...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not Config.Onlylazymen or (Config.Onlylazymen and ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "lazy") then
...
OpenlazyArmory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "lazy_armory_menu",
		{
			title    = "lazy Armory",
			align    = "center",
			elements = elements
...
OpenWeaponStorage = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "lazy_armory_weapon_menu",
		{
			title    = "lazy Weapon Armory",
			align    = "center",
			elements = elements
		},
	function(data, menu)
		...
		if DoesEntityExist(closestPed) and closestPedDst <= 5.0 then
			...
			TriggerServerEvent("lazyarmory:giveWeapon", weaponHash)
		end
...
RefreshPed = function(spawn)
	...
	ESX.TriggerServerCallback("lazyarmory:pedExists", function(Exists)
